...
connection.on('like', (msg) => {
    ...
    if (typeof msg.likeCount === 'number') {
        addChatItem('#fbf4bb', msg, msg.label.replace('{0:user}', '').replace('likes', `${msg.likeCount} likes`))
    }
})
...
connection.on('member', (msg) => {
    ...
    setTimeout(() => {
        joinMsgDelay -= addDelay;
        addChatItem('#fbf4bb', msg, 'joined', true);
    }, joinMsgDelay);
})
...
connection.on('social', (data) => {
    ...
    let color = data.displayType.includes('follow') ? '#ff005e' : '#ff9805';
